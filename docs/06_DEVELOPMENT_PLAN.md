...
| # | 작업 | 상세 |
|---|---|---|
| 추가-1 | Claude Vision API 실제 연동 | @anthropic-ai/sdk, claude-sonnet-4-20250514, tool_use 패턴, API 키 없으면 mock fallback |
| 추가-2 | 웹캠 촬영 기능 | WebCamera.tsx (브라우저 getUserMedia API), Platform.OS === "web"일 때 동작 |
| 추가-3 | 컨디션 체크 플로우 개선 | 단계별 로딩 화면, 시퀀스 생성 완료 후 오늘 탭 자동 이동, 촬영 이미지 미리보기 (웹) |
| 추가-4 | memberId 자동 조회 | POST /condition/register에서 로그인 사용자의 members 레코드 자동 조회 |
| 추가-5 | 시퀀스 재생성 시 기존 삭제 | POST /sequences/generate에서 같은 세션 기존 시퀀스 삭제 후 재생성 |
| 추가-6 | 50분 수업 기준 시퀀스 | 워밍업 3개(~7분) + 메인 8~12개(~36분) + 쿨다운 3개(~7분), 세트 간 휴식 15초 |
| 추가-7 | 오늘 탭 빈 상태 UX | "오늘의 컨디션을 체크해 주세요" + 버튼, success:true data:null 처리 |
| 추가-8 | 로그아웃 | 회원 프로필 탭 하단 + 강사 설정 탭 하단 (ScrollView) |
